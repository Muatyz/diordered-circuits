...
- [x] 成功完整下载 DANDI:000939/0.240528.1542
- [x] 生成 NWB inventory，知道有多少文件、session、subject、unit
- [x] 能说明数据内容：mouse postsubiculum, NWB, head direction, spike times, square exploration, opto periods excluded
- [x] 从至少一个 NWB 文件中提取 head_direction(t) 和 spike_times
- [x] 对一个 session 计算 occupancy histogram
- [x] 对单个 unit 画 raw tuning curve
- [x] 对所有可用 units 批量计算 100-bin raw tuning curves
- [x] 实现 split-half / 8-segment CV Gaussian smoothing
- [x] 保存 smoothed + unit-mean-normalized tuning matrix
