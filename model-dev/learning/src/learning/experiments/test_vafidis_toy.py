...
def run_tests_for_existing_run(
    *,
    ...
    if cached_weights_match and cached_trajectory_path.exists():
        cached_trajectory = load_npz(cached_trajectory_path)
        # The slow-manifold / timescale-separation diagnostics consume the
        # full autonomous Markov state captured along every darkness
        # trajectory (``autonomous_probe_state``, ``slow_candidate_state``,
        # ``ramesan_trajectory_state``).  Trajectories saved by an earlier
        # diagnostics config that had those groups disabled do NOT contain
        # these fields; reusing them would silently degrade the new
        # diagnostics.  The darkness duration must also match the configured
        # bump_attractor_duration (a 5 s cache must not serve a 20 s run).
        needs_autonomous_state = bool(
            config.diagnostics.pva_spectrum_and_visualization
            or config.diagnostics.timescale_separation
        )
        trajectory_cache_usable = True
        if needs_autonomous_state:
            probe_state = np.asarray(
                cached_trajectory.get(
                    "autonomous_probe_state", np.empty((0, 0))
                ),
                dtype=float,
            )
            trajectory_cache_usable = bool(
                probe_state.ndim == 2 and probe_state.shape[1] > 0
            )
        cached_time = np.asarray(
            cached_trajectory.get("time", np.empty(0)), dtype=float
        )
        if cached_time.size > 0:
            expected_duration = float(config.tests.bump_attractor_duration)
            sample_interval = float(
                config.tests.bump_attractor_sample_interval
            )
            cached_duration = float(cached_time[-1])
            trajectory_cache_usable = bool(
                trajectory_cache_usable
                and abs(cached_duration - expected_duration)
                <= sample_interval + 1e-9
            )
        if trajectory_cache_usable:
            cached_histories["bump_attractor_trajectories"] = cached_trajectory
