...
def plot_ramesan_phase_landscape(
    *,
    ...
    # Settled/moving decomposition (audit 2026-08-23).  Older histories lack
    # these fields; fall back to the raw median field in that case.
    settled_fraction = np.asarray(
        history.get("ramesan_phase_settled_fraction", np.full(phase.size, np.nan)),
        dtype=float,
    )
    moving_velocity_median = np.asarray(
        history.get(
            "ramesan_phase_moving_velocity_median", np.full(phase.size, np.nan)
        ),
        dtype=float,
    )
    moving_velocity_q25 = np.asarray(
        history.get(
            "ramesan_phase_moving_velocity_q25", np.full(phase.size, np.nan)
        ),
        dtype=float,
    )
    moving_velocity_q75 = np.asarray(
        history.get(
            "ramesan_phase_moving_velocity_q75", np.full(phase.size, np.nan)
        ),
        dtype=float,
    )
    has_moving_decomposition = bool(
        np.any(np.isfinite(settled_fraction))
        and np.any(np.isfinite(moving_velocity_median))
    )
    ...
    if has_moving_decomposition:
        # Settled bins: high settled fraction means the bump spends most of
        # the sampled time at that phase (an attracting phase).  Moving bins:
        # the median drift while the bump actually crosses the interval.  The
        # two regimes are separated so the coarse 0.1 s grid does not mix
        # many zeros with single-frame jumps.
        finite_settled = np.isfinite(settled_fraction)
        velocity_axis.fill_between(
            phase,
            np.zeros_like(phase),
            settled_fraction,
            where=finite_settled,
            color="#75d5d8",
            alpha=0.40,
            label="settled fraction (|v|<floor)",
        )
        velocity_axis.plot(
            phase,
            settled_fraction,
            ...
            linewidth=0.8,
            alpha=0.8,
            label="settled fraction",
        )
        finite_moving_band = (
            np.isfinite(moving_velocity_q25) & np.isfinite(moving_velocity_q75)
        )
        velocity_axis.fill_between(
            phase,
            moving_velocity_q25,
            moving_velocity_q75,
            where=finite_moving_band,
            color="#e58b35",
            alpha=0.28,
            label="moving-velocity IQR",
        )
        velocity_axis.plot(
            phase,
            moving_velocity_median,
            color="#d17c1f",
            linewidth=1.3,
            label="moving median (drift while crossing)",
        )
    else:
    ...
    velocity_axis.set_title(
        "B  Empirical zero-input phase field\n"
        + (
            "settled fraction [0,1] vs moving drift [rad/s]"
            if has_moving_decomposition
            else "raw within-bin median [rad/s]"
        )
    )
    velocity_axis.set_ylabel("settled fraction / " + r"$\dot{\theta}$ [rad/s]")
