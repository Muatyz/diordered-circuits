...
def _is_learning_project_root(candidate_path: Path) -> bool:
    has_project_marker = (candidate_path / "pyproject.toml").is_file() or (
        candidate_path / ".SKILL.md"
    ).is_file()
    return has_project_marker and (candidate_path / "src" / "learning").is_dir()


def find_project_root(start_path: str | Path | None = None) -> Path:
    """Find the active learning subproject without crossing snapshot trees."""
    current_path = Path(start_path or Path.cwd()).resolve()
    if current_path.is_file():
        current_path = current_path.parent
    for candidate_path in [current_path, *current_path.parents]:
        if _is_learning_project_root(candidate_path):
            return candidate_path
    package_project_root = Path(__file__).resolve().parents[3]
    if _is_learning_project_root(package_project_root):
        return package_project_root
    raise FileNotFoundError(f"Could not locate learning project root from {current_path}")


def find_repository_root(start_path: str | Path | None = None) -> Path:
    """Locate the repository root shared by model-dev / model-release trees.

    The learning subprojects (``model-dev/learning``, ``model-release/learning``,
    ``model-release-<series>/learning``) expose the same ``learning`` package but
    each resolves its own ``find_project_root``.  Run artifacts should be shared
    across all of them (one root ``runs/`` directory), so run directories are
    anchored at the *repository* root instead of the per-tree learning root.
    """
    current_path = Path(start_path or Path.cwd()).resolve()
    if current_path.is_file():
        current_path = current_path.parent
    for candidate_path in [current_path, *current_path.parents]:
        if (candidate_path / "model-dev").is_dir() and (
            candidate_path / "model-release"
        ).is_dir():
            return candidate_path
    # Fallback: the git working-tree root.
    try:
        import subprocess
    ...
    raise FileNotFoundError(
        f"Could not locate repository root from {current_path}"
    )
...
def _validate_diagnostics_mapping(
    ...
    *,
    ...
    selection_fields = {
        "reuse_cached_dependencies",
        "weight_source",
        "weight_snapshot_index",
    }
    unknown_fields = set(group_switches) - (DIAGNOSTIC_GROUPS | selection_fields)
    if missing_groups:
        raise ValueError(
    ...
    weight_source = group_switches.get("weight_source", "best")
    if weight_source not in {"best", "final", "training_selected", "snapshot"}:
        raise ValueError(
            "diagnostics.weight_source must be best, final, training_selected, "
            "or snapshot"
        )
    snapshot_index = group_switches.get("weight_snapshot_index")
    ...
    if weight_source == "snapshot" and snapshot_index is None:
        raise ValueError(
            "diagnostics.weight_snapshot_index is required when weight_source=snapshot"
        )
