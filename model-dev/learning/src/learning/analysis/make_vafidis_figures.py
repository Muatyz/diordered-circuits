...
def _pi_error_plot_data(
    history: dict[str, np.ndarray],
) -> tuple[np.ndarray, bool]:
    """Return the full-protocol PI error trace and whether to use a circular axis.

    Preferred source is the *wrapped* full-protocol error
    (``pi_error_full_protocol_wrapped``): the cue-darkness-recue window is
    unwrapped once, then folded back into ``[-pi, pi)``.  Wrapping matters for
    the recue segment: the visual anchor re-locks the decoded heading to the
    true heading, so the unwrapped accumulated error keeps a flat ``-2*pi``
    plateau even though the bump already follows the truth.  Folding removes
    that plateau and shows the error returning to zero when re-anchored.

    Fallback order when the wrapped field is absent (old archives):
    ``pi_error_full_protocol`` (continuous unwrapped) -> absolute circular
    decode error -> darkness-only ``pi_error_release_relative`` with the
    cue/recue NaN gaps filled by the instantaneous decode error.
    """

    if "pi_error_full_protocol_wrapped" in history:
        return (
            np.asarray(history["pi_error_full_protocol_wrapped"], dtype=float),
            True,
        )
    if "pi_error_full_protocol" in history:
        return (
            np.asarray(history["pi_error_full_protocol"], dtype=float),
            False,
        )
...
def _pi_error_peak_plot_data(
    history: dict[str, np.ndarray],
    circular: bool,
) -> np.ndarray | None:
    """Return the peak-decoder PI error trace aligned with the main plot mode."""
    peak = history.get("peak_pi_error_full_protocol")
    if peak is None:
        peak = history.get("peak_pi_error_release_relative")
    if peak is None:
        return None
    peak = np.asarray(peak, dtype=float)
    return np.asarray(wrap_angle(peak), dtype=float) if circular else peak


def _plot_pi_error_trace(history: dict[str, np.ndarray]) -> np.ndarray:
    """Backward-compatible single-trace accessor (unwrapped full protocol)."""
    trace, _circular = _pi_error_plot_data(history)
    return trace
...
def _make_grouped_diagnostic_figures(
    *,
    ...
    if groups & {
        ...
        darkness_error, darkness_error_circular = _pi_error_plot_data(
            darkness_history
        )
        darkness_peak_error = _pi_error_peak_plot_data(
            darkness_history, darkness_error_circular
        )
        plot_activity_heatmap(
        ...
        plot_heading_and_pi_error_panels(
            ...
            peak_pi_error=darkness_peak_error,
            circular_error_axis=darkness_error_circular,
            phase_id=darkness_history.get("phase_id"),
        ...
        plot_pi_error(
            ...
            circular_axis=darkness_error_circular,
        )
        constant_pi_keys = {
            "constant_pi_time",
            "constant_pi_error",
    ...
    if groups & {
        ...
        ou_error, ou_error_circular = _pi_error_plot_data(ou_history)
        ou_peak_error = _pi_error_peak_plot_data(ou_history, ou_error_circular)
        plot_activity_heatmap(
        ...
        plot_heading_and_pi_error_panels(
            ...
            theta_hd_decoded_peak=ou_history.get("theta_hd_decoded_peak"),
            pi_error=ou_error,
            peak_pi_error=ou_peak_error,
            circular_error_axis=ou_error_circular,
            phase_id=ou_history.get("phase_id"),
        ...
        plot_pi_error(
            ...
            title="OU PI error",
            circular_axis=ou_error_circular,
        )
    if groups & {
        "path_integration_and_pi_error",
