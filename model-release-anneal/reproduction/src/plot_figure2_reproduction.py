...
from workspace_paths import DATA_ROOT, REPRODUCTION_ROOT
...
def draw_zero_reference(ax):
    """
    Draw the aligned zero-degree guide without using Matplotlib's axvline.

    The local Windows/Matplotlib stack can crash inside `axvline` transform
    handling, so a regular data-coordinate line is used instead.
    """
    ymin, ymax = ax.get_ylim()
    ax.plot([0.0, 0.0], [ymin, ymax], color="#777777", linewidth=0.8, linestyle=":")
...
def load_aligned_subject_tuning(index_path=PROCESSED / "hd_tuning_index.csv"):
    """
    Load all processed sessions and COM-align included HD-cell tuning curves.
    ...
    for _, row in index.iterrows():
        ...
        angles_rad = data["bin_centers_rad"]
        ...
        aligned = align_rows_to_circular_com(matrix, angles_rad=angles_rad)
