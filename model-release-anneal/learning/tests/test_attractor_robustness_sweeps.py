...
def test_standard_run_figure_sentinels_use_diagnostic_group_directories() -> None:
    figure_paths = [
        Path(relative_path)
        for relative_path in STANDARD_RUN_FIGURE_SENTINELS
        if Path(relative_path).parts[0] == "figures"
    ]

    assert figure_paths
    assert all(path.parts[1] in DIAGNOSTIC_GROUPS for path in figure_paths)
