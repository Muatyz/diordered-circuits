...
class VafidisToyParams:
    ...
    training_integration_method: str
    plasticity_update_interval_duration: float
    tau_s: float
    tau_hd_to_hr: float
    tau_l_hd: float
    ...
    def from_config(cls, config: ExperimentConfig) -> "VafidisToyParams":
        ...
        if training_integration_method not in TRAINING_INTEGRATION_METHODS:
            raise ValueError(
                "simulation.training_integration_method must be one of "
                f"{sorted(TRAINING_INTEGRATION_METHODS)}"
            )
        plasticity_update_interval_duration = float(
            config.simulation.plasticity_update_interval_duration
        )
        if (
            not np.isfinite(plasticity_update_interval_duration)
            or plasticity_update_interval_duration <= 0.0
        ):
            raise ValueError(
                "simulation.plasticity_update_interval_duration must be "
                "finite and positive"
            )
        plasticity_block_steps = int(
            round(plasticity_update_interval_duration / dt)
        )
        if training_integration_method != TRAINING_INTEGRATION_SINGLE_CLOCK and (
            plasticity_block_steps < 1
            or not np.isclose(
                plasticity_block_steps * dt,
                plasticity_update_interval_duration,
                rtol=1e-9,
                atol=1e-12,
            )
        ):
            raise ValueError(
                "simulation.plasticity_update_interval_duration must be an "
                "integer multiple of simulation.dt"
            )
        ...
        proximal_euler_factor = (
            dt * total_proximal_conductance / c_hd_proximal
        )
        if (
        ...
        return cls(
            ...
            training_integration_method=training_integration_method,
            plasticity_update_interval_duration=(
                plasticity_update_interval_duration
            ),
...
def _make_i_vis_to_hd_proximal(
    *,
    ...
    visual_amplitude: float | None = None,
) -> np.ndarray:
    ...
    ``visual_amplitude`` overrides ``params.visual_amplitude`` for a single
    call.  The training loop uses it to apply the piecewise
    ``visual.training_amplitude_schedule`` without rebuilding frozen params.
    """
    visual_profile = params.visual_profile.lower()
    amplitude = params.visual_amplitude if visual_amplitude is None else float(visual_amplitude)
...
def step_vafidis_toy(
    *,
    ...
    copy_frozen_plasticity_state: bool = True,
) -> VafidisToyState:
    """Advance one step, with optional IID noise at all synaptic inputs.

    ``visual_amplitude`` overrides ``params.visual_amplitude`` for this step
    (used by the training amplitude schedule; diagnostics keep the configured
    amplitude).  ``validate_state=False`` is reserved for the interior
    microsteps of a multirate block. The block driver validates the complete
    state after its accumulated plasticity update. It also sets
    ``copy_frozen_plasticity_state=False`` because those arrays are immutable
    within the block and replaced atomically when plasticity is flushed.
    """
    ...
    if training:
        ...
        if copy_frozen_plasticity_state:
