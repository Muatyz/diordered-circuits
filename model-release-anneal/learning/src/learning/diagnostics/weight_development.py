...
def _validated_probe_settings(
...
) -> tuple[np.ndarray, np.ndarray, FrozenVelocityProbeSpec]:
    ...
    spec = FrozenVelocityProbeSpec(
        ...
        fit_start_time=float(settings.weight_snapshot_pi_average_start_time),
        velocity_during_cue=bool(
            settings.weight_snapshot_pi_velocity_during_cue
        ),
        minimum_moving_gain=float(
            settings.weight_snapshot_pi_minimum_moving_gain
        ),
    )
    spec.validate(dt=config.simulation.dt)
    return velocities, wrapped_headings, spec
...
def run_weight_snapshot_pi_development_diagnostic(
    *,
    ...
    velocities, initial_headings, probe_spec = _validated_probe_settings(config)
    (
        snapshot_time,
        w_hd_history,
        w_hr_history,
    ...
    detailed_decoded_velocity = np.empty(detailed_shape, dtype=float)
    detailed_velocity_gain = np.empty(detailed_shape, dtype=float)
    detailed_stalled = np.empty(detailed_shape, dtype=float)
    ...
    detailed_minimum_bump_contrast = np.empty(detailed_shape, dtype=float)
    summary_names = (
        "rms_velocity_bias",
    ...
    snapshot_summary = {
        summary_name: np.empty(snapshot_time.shape, dtype=float)
    ...
    for snapshot_index, snapshot_training_time in enumerate(snapshot_time):
        ...
        details, summary = run_frozen_velocity_probe_grid(
            config=config,
        ...
        detailed_time_averaged_abs_error[snapshot_index] = details[
            "time_averaged_abs_error"
        ]
        detailed_rms_error[snapshot_index] = details["rms_error"]
        detailed_final_abs_error[snapshot_index] = details["final_abs_error"]
        detailed_wrapped_time_averaged_abs_error[snapshot_index] = details[
            "wrapped_time_averaged_abs_error"
        ]
        detailed_wrapped_rms_error[snapshot_index] = details["wrapped_rms_error"]
        detailed_wrapped_final_abs_error[snapshot_index] = details[
            "wrapped_final_abs_error"
        ]
        detailed_velocity_bias[snapshot_index] = details["velocity_bias"]
        detailed_decoded_velocity[snapshot_index] = details["decoded_velocity"]
        detailed_velocity_gain[snapshot_index] = details["velocity_gain"]
        detailed_stalled[snapshot_index] = details["stalled"]
        detailed_mean_pva_strength[snapshot_index] = details["mean_pva_strength"]
        detailed_minimum_pva_strength[snapshot_index] = details[
            "minimum_pva_strength"
        ]
        detailed_mean_bump_contrast[snapshot_index] = details[
            "mean_bump_contrast"
        ]
        detailed_minimum_bump_contrast[snapshot_index] = details[
            "minimum_bump_contrast"
        ]
        for summary_name in summary_names:
            snapshot_summary[summary_name][snapshot_index] = summary[summary_name]
    ...
    maximum_abs_velocity_bias = snapshot_summary["maximum_abs_velocity_bias"]
    maximum_abs_zero_velocity_drift = snapshot_summary[
        "maximum_abs_zero_velocity_drift"
    ]
    depinning_velocity = snapshot_summary["depinning_velocity"]
    ...
    if selection_metric not in selection_score_by_name:
        raise ValueError(
            ...
            "mean_abs_unwrapped_error, rms_velocity_bias, "
            "maximum_abs_velocity_bias, or depinning_velocity"
        )
    selection_score = selection_score_by_name[selection_metric]
    threshold_names = (
        "weight_snapshot_pi_maximum_rms_velocity_bias",
        "weight_snapshot_pi_maximum_abs_velocity_bias",
        "weight_snapshot_pi_maximum_abs_zero_velocity_drift",
        "weight_snapshot_pi_maximum_depinning_velocity",
        "weight_snapshot_pi_minimum_pva_strength",
        "weight_snapshot_pi_minimum_bump_contrast",
    )
    for threshold_name in threshold_names:
        threshold_value = float(getattr(config.tests, threshold_name))
        if np.isnan(threshold_value) or threshold_value < 0.0:
            raise ValueError(f"tests.{threshold_name} must be non-negative")
    snapshot_acceptance_passed = np.asarray(
        (snapshot_summary["fully_defined"] > 0.5)
        & (
            aggregate_rms_velocity_bias
            <= config.tests.weight_snapshot_pi_maximum_rms_velocity_bias
        )
        & (
            maximum_abs_velocity_bias
            <= config.tests.weight_snapshot_pi_maximum_abs_velocity_bias
        )
        & (
            maximum_abs_zero_velocity_drift
            <= config.tests.weight_snapshot_pi_maximum_abs_zero_velocity_drift
        )
        & (
            depinning_velocity
            <= config.tests.weight_snapshot_pi_maximum_depinning_velocity
        )
        & (
            snapshot_summary["minimum_pva_strength"]
            >= config.tests.weight_snapshot_pi_minimum_pva_strength
        )
        & (
            snapshot_summary["minimum_bump_contrast"]
            >= config.tests.weight_snapshot_pi_minimum_bump_contrast
        ),
        dtype=bool,
    )
    ...
    eligible_indices = np.flatnonzero(snapshot_acceptance_passed)
    selection_was_fallback = eligible_indices.size == 0
    candidate_indices = (
        np.arange(snapshot_time.size, dtype=int)
        if selection_was_fallback
        else eligible_indices
    )
    finite_score = np.where(np.isfinite(selection_score), selection_score, np.inf)
    best_snapshot_index = int(
        candidate_indices[int(np.argmin(finite_score[candidate_indices]))]
    )
    ...
    history = {
        ...
        "decoded_velocity_by_initial_heading": detailed_decoded_velocity,
        "velocity_gain_by_initial_heading": detailed_velocity_gain,
        "stalled_by_initial_heading": detailed_stalled,
        "aggregate_rms_velocity_bias": aggregate_rms_velocity_bias,
        "maximum_abs_velocity_bias": maximum_abs_velocity_bias,
        "maximum_abs_zero_velocity_drift": maximum_abs_zero_velocity_drift,
        "negative_depinning_velocity": snapshot_summary[
            "negative_depinning_velocity"
        ],
        "positive_depinning_velocity": snapshot_summary[
            "positive_depinning_velocity"
        ],
        "depinning_velocity": depinning_velocity,
        "stall_fraction": snapshot_summary["stall_fraction"],
        "snapshot_acceptance_passed": snapshot_acceptance_passed.astype(float),
        "selection_was_fallback": np.asarray(
            float(selection_was_fallback), dtype=float
        ),
        ...
        "minimum_bump_contrast_by_initial_heading": (
            detailed_minimum_bump_contrast
        ),
    ...
    metrics = {
        ...
        "weight_snapshot_pi_selection_uses_maximum_abs_velocity_bias": float(
            selection_metric == "maximum_abs_velocity_bias"
        ),
        "weight_snapshot_pi_best_acceptance_passed": float(
            snapshot_acceptance_passed[best_snapshot_index]
        ),
        "weight_snapshot_pi_selection_was_fallback": float(
        ...
        "weight_snapshot_pi_best_maximum_abs_velocity_bias_deg_s": float(
            np.rad2deg(maximum_abs_velocity_bias[best_snapshot_index])
        ),
        "weight_snapshot_pi_final_maximum_abs_velocity_bias_deg_s": float(
            np.rad2deg(maximum_abs_velocity_bias[-1])
        ),
        "weight_snapshot_pi_best_depinning_velocity_deg_s": float(
            np.rad2deg(depinning_velocity[best_snapshot_index])
        ),
        "weight_snapshot_pi_final_depinning_velocity_deg_s": float(
            np.rad2deg(depinning_velocity[-1])
        ),
