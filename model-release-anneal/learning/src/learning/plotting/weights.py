...
def plot_weight_snapshot_pi_development(
    *,
    ...
    maximum_abs_velocity_bias: np.ndarray | None = None,
    depinning_velocity: np.ndarray | None = None,
    snapshot_acceptance_passed: np.ndarray | None = None,
    selection_was_fallback: bool | None = None,
    ...
    if snapshot_acceptance_passed is not None:
        normalized_acceptance = np.asarray(
            snapshot_acceptance_passed, dtype=bool
        )
        if normalized_acceptance.shape != snapshot_time.shape:
            raise ValueError("snapshot_acceptance_passed must match snapshot_time")
    ...
    metric_label = {
        ...
        "maximum_abs_velocity_bias": "worst-case velocity bias",
        "depinning_velocity": "worst-case depinning speed",
    }.get(str(selection_metric), str(selection_metric or "configured score"))
    selection_status = (
        " (fallback: no snapshot passed)"
        if selection_was_fallback
        else " (accepted-first)"
        if selection_was_fallback is not None
        else ""
    )
    best_label = (
    ...
    if show_joint_diagnostics:
        ...
        worst_bias = normalized_optional.get("maximum_abs_velocity_bias")
        if worst_bias is not None:
            bias_axis.plot(
                snapshot_time,
                np.rad2deg(worst_bias),
                color="#d65f5f",
                linewidth=1.6,
                label="maximum |velocity bias|",
            )
        depinning = normalized_optional.get("depinning_velocity")
        if depinning is not None:
            finite_depinning = np.where(np.isfinite(depinning), depinning, np.nan)
            bias_axis.plot(
                snapshot_time,
                np.rad2deg(finite_depinning),
                color="#e09020",
                linewidth=1.4,
                linestyle="--",
                label="worst-case depinning speed",
            )
        if normalized_acceptance is not None and np.any(normalized_acceptance):
            marker_source = (
                worst_bias
                if worst_bias is not None
                else velocity_bias
            ...
            if marker_source is not None:
                bias_axis.scatter(
                    snapshot_time[normalized_acceptance],
                    np.rad2deg(marker_source[normalized_acceptance]),
                    color="#18864b",
                    marker="o",
                    s=24,
        ...
        bias_axis.set_ylabel("bias / depinning [deg/s]")
