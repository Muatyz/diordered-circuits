...
def full_protocol_pi_error_trace(
    theta_decoded: np.ndarray,
    theta_true: np.ndarray,
    *,
    ...
    """Return accumulated PI error over the full cue-darkness-recue protocol.

    Unlike :func:`release_relative_pi_error_trace`, which is referenced to
    (and only meaningful over) the darkness phase, this helper unwraps the
    whole visual-dark-visual window in one continuous sweep and measures the
    integrator claim relative to a single reference sample (default: the
    first sample of the protocol, typically darkness onset or protocol
    start):

    ``(decoded(t) - decoded(t_ref)) - (true(t) - true(t_ref))``

    The single unwrap keeps the trace continuous across phase transitions, so
    the cue and recue segments show whether a visual anchor holds or re-anchors
    the accumulated error toward zero.
    """
    ...
    if reference_index < 0 or reference_index >= decoded.shape[normalized_axis]:
        raise ValueError("reference_index must index the trace axis")
    decoded_unwrapped = np.unwrap(decoded, axis=normalized_axis)
    true_unwrapped = np.unwrap(truth, axis=normalized_axis)
    ref_slice = [slice(None)] * decoded.ndim
    ref_slice[normalized_axis] = slice(reference_index, reference_index + 1)
    decoded_displacement = decoded_unwrapped - decoded_unwrapped[tuple(ref_slice)]
    true_displacement = true_unwrapped - true_unwrapped[tuple(ref_slice)]
    return np.asarray(decoded_displacement - true_displacement, dtype=float)
