...
def sum_activity_by_theta(
    theta_hd_pref: np.ndarray,
    activity: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    """Sum activity over cells sharing a preferred direction.

    The final activity axis indexes HD cells; any preceding axes are retained.
    Summation, rather than averaging, preserves the exact complex first moment
    used by PVA when Vafidis odd/even partners share an angular preference.
    """
    theta_hd_pref = np.asarray(theta_hd_pref, dtype=float)
    activity = np.asarray(activity, dtype=float)
    if theta_hd_pref.ndim != 1 or activity.ndim < 1:
        raise ValueError("theta_hd_pref must be 1D and activity must have an axis")
    if activity.shape[-1] != theta_hd_pref.size:
        raise ValueError("the final activity axis must match theta_hd_pref")
    unique_theta, inverse_index = np.unique(theta_hd_pref, return_inverse=True)
    angular_activity = np.zeros(
        (*activity.shape[:-1], unique_theta.size),
        dtype=float,
    )
    for angular_index in range(unique_theta.size):
        angular_activity[..., angular_index] = np.sum(
            activity[..., inverse_index == angular_index],
            axis=-1,
        )
    return unique_theta, angular_activity
