...
def spike_counts_by_hd(spike_times, behavior, n_bins):
    ...
    hd_bin, _ = bin_head_direction(hd, n_bins)
    spike_bin = hd_bin[interval_idx[keep]]
...
def choose_poisson_cv_sigmas(counts_a, counts_b, occ_a, occ_b, min_occupancy_s, sigma_candidates):
    """
    Select smoothing widths by cross-partition Poisson likelihood.

    For each unit and candidate width, partition A is smoothed and evaluated
    on partition B, while partition B is smoothed and evaluated on partition A.
    The single width maximizing that unit's summed bidirectional likelihood is
    then used to smooth both partitions before averaging, matching Methods B2's
    per-neuron smoothing description.
    """
    n_units = counts_a.shape[0]
    selected_sigma = np.full(n_units, np.nan)
    ...
    for unit_i in range(n_units):
        ...
        score = scores_a_to_b[unit_i] + scores_b_to_a[unit_i]
        if np.isfinite(score).any():
            selected_sigma[unit_i] = sigma_candidates[int(np.nanargmax(score))]

    return selected_sigma, scores_a_to_b, scores_b_to_a, rate_a, rate_b
...
def process_session(row, n_bins, min_occupancy_s, sigma_candidates):
    ...
    chosen_sigma, cv_a_to_b, cv_b_to_a, rate_a, rate_b = choose_poisson_cv_sigmas(
        counts2[0],
        counts2[1],
        occ2[0],
        occ2[1],
    ...
    smoothed_a = np.vstack(
        [circular_smooth(rate_a[i], chosen_sigma[i] if np.isfinite(chosen_sigma[i]) else 2.0) for i in range(len(unit_ids))]
    )
    smoothed_b = np.vstack(
        [circular_smooth(rate_b[i], chosen_sigma[i] if np.isfinite(chosen_sigma[i]) else 2.0) for i in range(len(unit_ids))]
    )
