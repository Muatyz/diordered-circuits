...
def circular_smooth(values, sigma_bins):
    """
    Smooth circular tuning curves with a Gaussian kernel.

    The last axis is treated as angular bins on a closed circle, so values
    near 0 and 360 degrees are neighbors. `sigma_bins <= 0` returns a float
...
def softplus_inverse(rate, beta=2.0, eps=1e-8):
    """
    ...
    are clipped away from zero so the inverse remains finite. The data-derived
    network model uses beta=2.
    """
...
def nearest_manifold_distance(states, manifold, return_l2=False):
    ...
    diff = states[:, None, :] - manifold[None, :, :]
    l2 = np.linalg.norm(diff, axis=2)
    nearest = np.argmin(l2, axis=1)
    nearest_l2 = l2[np.arange(len(states)), nearest]
...
def simulate_rate_network_with_drive(
    ...
    firing-rate states shaped `(n_trials, n_neurons)`.
    """
