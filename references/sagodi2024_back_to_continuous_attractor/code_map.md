...
| Fixed points and basins | `analysis/metrics.py::classify_endpoint_map_fixed_points`, `analysis/slow_manifold.py`, `phase_flow.py::actual_stable_basins` | Long-run endpoint displacement now uses periodic signed roots (`+→-` stable, `-→+` unstable), reports cue-map validity and does not force alternating pairs; phase-flow roots remain an independent assay | Cross-check endpoint roots against zero-input phase-flow roots on coverage-qualified rings |
