...
def _make_grouped_diagnostic_figures(
    *,
    ...
    if groups & {
        ...
        darkness_error = darkness_history.get(
            "pi_error_release_relative",
            circular_error_trace(
    ...
    if "trajectory_and_fixed_points" in groups:
        ...
        plot_bump_attractor_pva_trajectories(
            ...
            path=heading_dir / "bump_attractor_pva_trajectories.png",
        )
...
def make_vafidis_figures_for_run(*, run_dir: str | Path) -> dict:
    ...
    figure_jobs: list[tuple[str, str]] = []
    for group_name in sorted(selected_groups):
        if group_name == "path_integration_and_pi_error":
            figure_jobs.extend(
                [
                    ("path_integration.constant", "path_integration_constant"),
                    ("path_integration.ou", "path_integration_ou"),
                    ("path_integration.ensemble", "path_integration_ensemble"),
                ]
            )
        else:
            figure_jobs.append((group_name, group_name))
    ...
    flush_figure_status()
    for status_name, internal_group_name in figure_jobs:
        figure_status["figures"][status_name] = {"status": "running"}
        flush_figure_status()
        try:
            ...
            figure_status["figures"][status_name] = {"status": "completed"}
        flush_figure_status()
    ...
    if (
        ...
        plot_bump_attractor_decoder_trajectories(
            ...
            cue_time=bump_attractor_trajectory_history.get("cue_time"),
            cue_theta_pva=bump_attractor_trajectory_history.get(
                "cue_theta_pva"
            ),
            cue_theta_peak=bump_attractor_trajectory_history.get(
                "cue_theta_peak"
            ),
            cue_theta_overlap=bump_attractor_trajectory_history.get(
                "cue_theta_overlap"
            ),
            endpoint_probe_theta_initial=bump_attractor_trajectory_history.get(
                "endpoint_probe_theta_initial"
            ),
            endpoint_probe_refinement_level=bump_attractor_trajectory_history.get(
                "endpoint_probe_refinement_level"
            ),
            endpoint_probe_theta_release_pva=bump_attractor_trajectory_history.get(
                "endpoint_probe_theta_release_pva"
            ),
            endpoint_probe_theta_release_peak=bump_attractor_trajectory_history.get(
                "endpoint_probe_theta_release_peak"
            ),
            endpoint_probe_theta_release_overlap=bump_attractor_trajectory_history.get(
                "endpoint_probe_theta_release_overlap"
            ),
            endpoint_probe_theta_final_pva=bump_attractor_trajectory_history.get(
                "endpoint_probe_theta_final_pva"
            ),
            endpoint_probe_theta_final_peak=bump_attractor_trajectory_history.get(
                "endpoint_probe_theta_final_peak"
            ),
            endpoint_probe_theta_final_overlap=bump_attractor_trajectory_history.get(
                "endpoint_probe_theta_final_overlap"
            ),
            path=heading_dir / "bump_attractor_decoder_trajectories.png",
        )
    ...
    if bump_diffusion_history is not None:
        ...
        if (
            "test_noise_std" in bump_diffusion_history
            and "pva_endpoint_diffusion_coefficient" in bump_diffusion_history
        ):
            plot_diffusion_noise_sweep(
                noise_std=bump_diffusion_history["test_noise_std"],
                diffusion_coefficient=bump_diffusion_history[
                    "pva_endpoint_diffusion_coefficient"
                ],
                path=diagnostics_dir / "bump_diffusion_vs_test_noise.png",
            )
