...
def _fit_frozen_checkpoint_probe_velocity(
    *,
    config: ExperimentConfig,
    ...
    """Return decoded velocity and minimum bump quality for one probe.

    The visual cue is stationary.  Velocity starts only after cue removal, so
    the fit measures the frozen network's darkness response rather than a
    mixture of visual entrainment and autonomous path integration.  Regression
    sums are accumulated online to avoid retaining full validation histories.
    """
    settings = config.simulation.checkpoint_selection
    ...
    cue_steps = int(round(float(settings.cue_duration) / params.dt))
    ...
    probe_steps = max(1, int(round(float(settings.probe_duration) / params.dt)))
    fit_start_step = int(round(float(settings.fit_start_time) / params.dt))
    previous_angle = float(state.theta_hd_decoded)
    unwrapped_angle = previous_angle
    sample_count = 0
    sum_time = 0.0
    sum_angle = 0.0
    sum_time_squared = 0.0
    sum_time_angle = 0.0
    minimum_strength = float("inf")
    ...
    for probe_step in range(1, probe_steps + 1):
        ...
        decoded_angle = float(state.theta_hd_decoded)
        unwrapped_angle += float(circular_difference(decoded_angle, previous_angle))
        previous_angle = decoded_angle
        current_strength = pva_vector_strength(state.theta_hd_pref, state.r_hd)
        minimum_strength = (
            min(minimum_strength, current_strength)
            if np.isfinite(current_strength)
            else float("-inf")
        )
        minimum_contrast = min(
            minimum_contrast,
            float(np.max(state.r_hd) - np.min(state.r_hd)),
        )
        if probe_step >= fit_start_step:
            sample_time = probe_step * params.dt
            sample_count += 1
            sum_time += sample_time
            sum_angle += unwrapped_angle
            sum_time_squared += sample_time * sample_time
            sum_time_angle += sample_time * unwrapped_angle

    denominator = sample_count * sum_time_squared - sum_time * sum_time
    decoded_velocity = (
        (sample_count * sum_time_angle - sum_time * sum_angle) / denominator
        if sample_count >= 2 and denominator > 0.0
        else float("nan")
    )
    return float(decoded_velocity), minimum_strength, minimum_contrast
...
def evaluate_training_checkpoint_behavior(
    *,
    ...
    settings = config.simulation.checkpoint_selection
    velocity_biases: list[float] = []
    zero_velocity_drifts: list[float] = []
    minimum_pva_strength = float("inf")
    minimum_bump_contrast = float("inf")
    for angular_velocity in settings.velocities:
        for theta_initial in settings.initial_headings:
            decoded_velocity, pva_strength, bump_contrast = (
                _fit_frozen_checkpoint_probe_velocity(
                    config=config,
                    trained_state=trained_state,
            ...
            velocity_biases.append(decoded_velocity - float(angular_velocity))
            if abs(float(angular_velocity)) <= 1e-12:
                zero_velocity_drifts.append(decoded_velocity)
            minimum_pva_strength = min(minimum_pva_strength, pva_strength)
            minimum_bump_contrast = min(minimum_bump_contrast, bump_contrast)

    velocity_bias = np.asarray(velocity_biases, dtype=float)
    zero_velocity_drift = np.asarray(zero_velocity_drifts, dtype=float)
    finite = np.isfinite(velocity_bias)
    fully_defined = bool(np.all(finite))
    rms_velocity_bias = (
        float(np.sqrt(np.mean(np.square(velocity_bias[finite]))))
        if np.any(finite)
        else float("inf")
    )
    maximum_abs_velocity_bias = (
        float(np.max(np.abs(velocity_bias[finite])))
        if np.any(finite)
        else float("inf")
    )
    maximum_abs_zero_velocity_drift = (
        float(np.max(np.abs(zero_velocity_drift[np.isfinite(zero_velocity_drift)])))
        if np.any(np.isfinite(zero_velocity_drift))
        else float("inf")
    )
    acceptance_passed = bool(
        fully_defined
        and rms_velocity_bias <= float(settings.maximum_rms_velocity_bias)
        and maximum_abs_velocity_bias <= float(settings.maximum_abs_velocity_bias)
        and maximum_abs_zero_velocity_drift
        <= float(settings.maximum_abs_zero_velocity_drift)
        and minimum_pva_strength >= float(settings.minimum_pva_strength)
        and minimum_bump_contrast >= float(settings.minimum_bump_contrast)
    )
    return {
        "score": rms_velocity_bias,
        ...
        "minimum_bump_contrast": float(minimum_bump_contrast),
    ...
    for step_index in progress_iterator:
        ...
        if run_checkpoint_selection:
            ...
            quality_eligible = bool(
                behavioral_metrics["fully_defined"]
                and behavioral_metrics["minimum_pva_strength"]
                >= float(checkpoint_selection.minimum_pva_strength)
                and behavioral_metrics["minimum_bump_contrast"]
                >= float(checkpoint_selection.minimum_bump_contrast)
            )
            candidate_score = (
            ...
            for metric_name in (
                ...
                checkpoint_selection_history[metric_name].append(
                    float(behavioral_metrics[metric_name])
                )
...
def run_bump_attractor_trajectory_test(
    *,
    ...
    def simulate_endpoint_probe(
        ...
        for _darkness_step in range(darkness_steps):
            state = step_vafidis_toy(
                state=state,
                params=params,
                angular_velocity=0.0,
