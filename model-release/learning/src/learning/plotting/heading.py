...
def plot_bump_attractor_pva_trajectories(
    *,
    ...
    """Plot the canonical three-panel PVA attractor diagnostic.

    The panels deliberately separate three scientific questions: autonomous
    trajectories after cue release, cue-to-release transfer, and the endpoint
    map.  Cue onset and release-to-endpoint auxiliary maps are excluded so the
    identity comparisons cannot be confused with one another.
    """

    del cue_time, endpoint_probe_refinement_level
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    time = np.asarray(time, dtype=float)
    theta_initial = np.asarray(theta_initial, dtype=float)
    ...
    trajectory_axis, cue_axis, endpoint_axis = axes
    for trace in theta_pva:
        trajectory_axis.plot(
            time,
    ...
    trajectory_axis.set_title("autonomous darkness trajectories")

    identity = np.asarray([-180.0, 180.0])
    cue_axis.scatter(
        np.rad2deg(wrap_angle(theta_initial)),
        np.rad2deg(wrap_angle(theta_release)),
        s=7.0,
        color="#2f6f9f",
        alpha=0.68,
        linewidths=0.0,
    ...
    cue_axis.set_xlabel("initial cue angle [deg]")
    cue_axis.set_ylabel("relaxed angle at cue release [deg]")
    cue_axis.set_title("cue transfer")

    endpoint_axis.scatter(
        np.rad2deg(wrap_angle(probe_initial)),
    ...
    marker_spec = {
        -1: ("o", "#17becf", "stable FP"),
        1: ("X", "#f28e2b", "unstable FP"),
    }
    for stability, (marker, color, label) in marker_spec.items():
        mask = fixed_stability == stability
        if np.any(mask):
            endpoint_axis.scatter(
                np.rad2deg(wrap_angle(fixed_initial[mask])),
                np.rad2deg(wrap_angle(fixed_theta[mask])),
                s=38.0,
                marker=marker,
                color=color,
                edgecolors="black" if stability == -1 else "none",
                linewidths=0.45,
                zorder=4,
                label=label,
            )
    ...
    endpoint_axis.set_xlabel("initial cue angle [deg]")
    endpoint_axis.set_ylabel("darkness endpoint angle [deg]")
    endpoint_axis.set_title("endpoint map and fixed points")
    ...
    cue_axis.set_xlim(-180.0, 180.0)
    endpoint_axis.set_xlim(-180.0, 180.0)
    cue_axis.set_xticks(major_ticks)
    endpoint_axis.set_xticks(major_ticks)
    cue_axis.legend(frameon=False, fontsize=8, loc="upper left")
