...
from learning.common.angles import pva_decode, sum_activity_by_theta
...
class FrozenAutonomousDynamics:
    ...
    def pva_angular_rate_state(
        self,
        state_vector: np.ndarray,
    ) -> tuple[np.ndarray, np.ndarray]:
        """Return the exact paired-HD angular-rate observable entering PVA.

        HD rate is derived from the dynamic proximal voltage in the canonical
        Markov state.  Cells sharing a preferred direction are summed because
        that sum, rather than the full state or a PCA projection, is the
        sufficient statistic entering the PVA complex first moment.
        """
        return sum_activity_by_theta(
            self.theta_hd_pref,
            self.hd_rate(state_vector),
        )

    def decoded_heading(self, state_vector: np.ndarray) -> float:
        theta_pref, angular_rate = self.pva_angular_rate_state(state_vector)
        return pva_decode(theta_pref, angular_rate)
