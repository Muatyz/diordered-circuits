...
def test_sum_activity_by_theta_preserves_batched_pva_first_moment() -> None:
    ...
    unique_theta, angular_activity = sum_activity_by_theta(
        theta_hd_pref,
        activity,
    )
    direct_vector = activity @ np.exp(1j * theta_hd_pref)
    grouped_vector = angular_activity @ np.exp(1j * unique_theta)
...
def test_peak_decode_handles_plateau_across_angle_boundary() -> None:
    theta_hd_pref = make_theta_hd_pref(8)
    r_hd = np.zeros(8)
    r_hd[0] = 1.0
    r_hd[-1] = 1.0
    decoded_angle = peak_decode(theta_hd_pref, r_hd)
    assert np.isclose(decoded_angle, 7.0 * np.pi / 8.0)


def test_peak_decode_rejects_ambiguous_separated_peak_tops() -> None:
    theta_hd_pref = make_theta_hd_pref(8)
    r_hd = np.zeros(8)
    r_hd[1] = 1.0
    r_hd[5] = 1.0
    decoded_angle = peak_decode(theta_hd_pref, r_hd)
    assert np.isnan(decoded_angle)
