...
def _sum_hd_rates_by_preferred_direction(
    *,
    ...
    unique_theta, inverse_index = np.unique(theta_hd_pref, return_inverse=True)
    angular_rate = np.zeros((hd_rate.shape[0], unique_theta.size), dtype=float)
    for angular_index in range(unique_theta.size):
        angular_rate[:, angular_index] = np.sum(
            hd_rate[:, inverse_index == angular_index], axis=1
        )
    return unique_theta, angular_rate
...
def select_slow_candidate_indices(
    *,
    ...
    """Select points below a trajectory-relative speed threshold.

    Ságodi et al. use one thousandth of the maximum speed along each
    trajectory.  When more points qualify than the storage budget, evenly
    spaced indices are retained so the result is not biased to one tail end.
    """
    ...
    threshold = speed_fraction * float(np.max(speed))
...
def analyze_ramesan_firing_rate_geometry(
    *,
    ...
    uniformly sampled cue phase.  PCA is fitted only to the concatenated HD+HR
    firing rates for visualization; flow, q, tangent projection, and the
    Jacobian are all evaluated in the full canonical Markov state.
    """
    ...
    firing_rate_center = np.asarray(firing_rate_pca["center"], dtype=float)
    centered_probe_rate = probe_firing_rate - firing_rate_center
    right = np.asarray(firing_rate_pca["components"], dtype=float)
    singular_value = np.asarray(firing_rate_pca["singular_value"], dtype=float)
    ...
    retained_component_count = min(3, right.shape[0])
    pca_components = np.zeros((3, probe_firing_rate.shape[1]), dtype=float)
    pca_components[:retained_component_count] = right[:retained_component_count]
    probe_pc = centered_probe_rate @ pca_components.T
    ...
    pva_theta_pref, pva_angular_rate = _sum_hd_rates_by_preferred_direction(
        theta_hd_pref=dynamics.theta_hd_pref,
        hd_rate=probe_firing_rate[:, : dynamics.params.n_theta],
    )
    ...
    if candidate_state.shape[0]:
        ...
        candidate_pc = (candidate_firing_rate - firing_rate_center) @ pca_components.T
    else:
...
def _sample_ambient_q_tube(
    *,
    ...
    for current_slice in dynamics.component_slices.values():
        block_scale = float(np.std(base_state[:, current_slice]))
        if not np.isfinite(block_scale) or block_scale < 1e-6:
            block_scale = max(
                float(np.sqrt(np.mean(np.square(base_state[:, current_slice])))),
                1e-3,
            )
    ...
    with progress:
        for scale, current_count in zip(perturbation_scales, samples_per_scale):
            ...
            for base_position in base_positions:
                ...
                firing_rate = dynamics.firing_rate_state(perturbed_state)
...
def analyze_ramesan_phase_landscape(
    *,
    ...
    for current_bin in np.flatnonzero(sample_count):
        ...
        phase_velocity_median[current_bin] = float(
            np.median(instantaneous_velocity[current_mask])
        )
        phase_velocity_q25[current_bin], phase_velocity_q75[current_bin] = (
            np.quantile(instantaneous_velocity[current_mask], [0.25, 0.75])
        )

    smoothed_velocity = _fill_and_smooth_periodic_scalar(
        phase_velocity_median,
