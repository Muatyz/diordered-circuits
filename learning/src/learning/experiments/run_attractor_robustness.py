...
STANDARD_RUN_FIGURE_SENTINELS = [
    ...
    "figures/activity/training_hd_activity_heatmap.png",
    "figures/activity/training_visual_input_heatmap.png",
    "figures/activity/darkness_hd_activity_heatmap.png",
    "figures/heading/darkness_true_vs_decoded_heading.png",
    "figures/heading/darkness_pi_error.png",
    "figures/weights/training_weight_matrices_side_by_side.png",
    "figures/gain/velocity_gain_curve.png",
    "figures/diagnostics/bump_effective_diffusion_msd.png",
]
...
def _resolve_path_argument(
    ...
    *,
    ...
    candidate_roots.extend([project_root, Path.cwd(), Path.cwd() / "learning"])
...
def _resolve_existing_cli_path(path_value: str) -> Path:
    ...
    if path.is_absolute():
        return path.resolve()
    for candidate_root in [Path.cwd(), Path.cwd() / "learning"]:
        candidate_path = candidate_root / path
        if candidate_path.exists():
            return candidate_path.resolve()
    return path.resolve()
...
def _clone_config_with_runtime_overrides(
    ...
    *,
    ...
    if save_interval_steps is not None:
        config_copy.simulation.save_interval_steps = int(save_interval_steps)
    return config_copy
...
def main() -> None:
    ...
    parser.add_argument("--save-interval-steps", type=int, default=None, help="Override save interval for sweeps.")
    ...
    save_interval_steps = (
        ...
        if args.save_interval_steps is not None
            ...
            _nested_get(
                robustness_config,
                ("runtime", "save_interval_steps"),
                robustness_config.get("save_interval_steps"),
            )
        )
    )
    progress = (
