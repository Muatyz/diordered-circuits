...
def plot_ramesan_pca_variance_rank(
    *,
    ...
    """Plot complete PCA spectra for the PVA statistic and HD+HR rate state."""
    rate_explained = np.asarray(
        history["ramesan_pca_explained_variance_spectrum"], dtype=float
    )
    rate_cumulative = np.asarray(
    ...
    for label, explained, cumulative in (
        ("HD+HR firing-rate state", rate_explained, rate_cumulative),
        ("PVA angular HD statistic", pva_explained, pva_cumulative),
    ):
    ...
    cumulative_axis.set_title(
        ...
        f"PC1–3: HD+HR={100.0 * np.sum(rate_explained[:3]):.1f}%, "
        f"PVA statistic={100.0 * np.sum(pva_explained[:3]):.1f}%"
    )
    cumulative_axis.grid(alpha=0.2)
    cumulative_axis.legend(fontsize=8, loc="lower right")

    figure.suptitle(
        "Linear PCA dimensionality of uniformly cue-sampled firing-rate geometry\n"
        "The three-PC projection is visualization only; q and Jacobians remain full-state",
        fontsize=12,
    )
    path = Path(path)
...
def plot_ramesan_firing_rate_diagnostics(
    *,
    ...
    """Plot firing-rate PC geometry, q, tangent flow, and local lambda_max."""
    ...
    ring_axis.set_title(
        "A  Cue-defined firing-rate state ring\n"
    ...
    figure.suptitle(
        ...
        "PCA is visualization only; q, tangent flow, and Jacobian use the full Markov state",
        fontsize=13,
    )
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
